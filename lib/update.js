...
exports.update = function update(callback) {
	...
	fs.readFile(exports.before, 'utf8', function reading(err, before) {
		...
		axios.get(exports.remote).then(function (response) {
			if(response.status !==  200) return callback(new Error('Invalid statusCode returned'))
			...
			fs.readFile(exports.after, 'utf8', function reading(err, after) {
				...
				tmp.file(function (err, tempFilePath) {
					...
					var writeStream=fs.createWriteStream(exports.output);
		...
		for (var i = 0, l = resources.length; i < l; i++) {
			...
			if(details.name == "device") {
				...
				if (resource.brand_replacement) {
					//console.log(resource.brand_replacement);
					parser += 'parser[5] = "'+ resource.brand_replacement.replace('"', '\\"') +'";';
				} else {
					parser += 'parser[5] = "Other";';
				}

				parser += '\n';

				if (resource.model_replacement) {
				//console.log(resource.model_replacement);
				parser += 'parser[6] = "'+ resource.model_replacement.replace('"', '\\"') +'";';
				} else {
				parser += 'parser[6] = "Other";';
				}
			}
